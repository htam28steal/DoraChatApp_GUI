  ...
  useEffect(() => {
    const handleNewMessage = (message) => {
      ...
      setConversations((prev) => {
        // find index of the conv to update
        const idx = prev.findIndex((c) => c._id === message.conversationId);
        if (idx < 0) return prev; // nothing to do

        // build an updated version of that convo
        const updatedConv = {
          ...prev[idx],
          lastMessageId: message,
        };

        // move it to front, preserving the rest
    ...
    return () => socket.off(SOCKET_EVENTS.RECEIVE_MESSAGE, handleNewMessage);
  }, []);
    ...
    let convs =
      selectedFilters.length > 0
        ? base.filter(
            (c) =>
              c &&
              selectedFilters.some((tagId) =>
                (
                  classifies.find((t) => t._id === tagId)?.conversationIds || []
                ).includes(c._id)
              )
          )
        : base;
    ...
    try {
      ...
      const normalized = latest.map((c) => ({
        ...
        conversationIds: (c.conversationIds || []).map((idObj) =>
          typeof idObj === "object" && idObj.$oid ? idObj.$oid : String(idObj)
        ),
      }));
  ...
  const openClassifyModal = async () => {
    try {
      ...
      if (!token) {
        Alert.alert(
          "Missing token",
          "Cannot open classify modal because userToken is missing."
        );
        return;
      }
        ...
        >
          <View style={styles.fMessage}>
            ...
            <View style={styles.fInfor}>
              <Text style={styles.name}>{other.name || "Unnamed"}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <Text style={styles.placeholderText}>No conversations found.</Text>
      ) : (
        <FlatList
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.menuModalContent}>
            <TouchableOpacity
              onPress={openClassifyOptions}
              style={styles.menuItem}
            >
              <Text style={styles.menuText}>Classify as</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => setClassifyMenuVisible(false)}
              style={styles.menuItem}
            >
                        ...
                        Alert.alert(
                          ...
                          [
                            { text: "Hủy", style: "cancel" },
                            {
                              text: "Xóa",
                              style: "destructive",
                              onPress: () => deleteClassify(item._id),
                            },
                          ]
                        )
                      }
        ...
        <View style={styles.modalContainer}>
          <View style={styles.addTagModal}>
            ...
            <Text style={styles.label}>
              Tên thẻ phân loại <Text style={{ color: "red" }}>*</Text>
            </Text>
            <TextInput
              style={styles.input}
            ...
            <Text style={styles.label}>
              Chọn màu <Text style={{ color: "red" }}>*</Text>
            </Text>
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                if (item.type) {
                  ...
                  // single chat
                  const other = Array.isArray(item.members)
                    ? item.members.find((m) => m.userId !== userId)
                    : {};
                  displayName =
                    friendsById[other.userId]?.name || other.name || "Unknown";
